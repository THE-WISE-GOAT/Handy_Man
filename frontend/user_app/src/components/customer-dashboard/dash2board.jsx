    ...
    return (
      <div style={modalStyle} onClick={handleBackdropClick}>
        <div style={contentStyle}>
          ...
          {selectedWorkersForReview.length === 0 ? (
            <p style={{ color: "var(--k-ink-3)" }}>No workers selected for review.</p>
          ) : (
            selectedWorkersForReview.map((worker, idx) => (
              <div key={idx} style={{ marginBottom: "16px", padding: "12px", border: "1px solid var(--k-line)", borderRadius: "8px" }}>
                <strong>{worker.username || `Worker ${worker.worker_chat_id}`}</strong>
                <p style={{ margin: "4px 0", fontSize: "13px", color: "var(--k-ink-3)" }}>
                  {worker.job_description || "No description available."}
                </p>
                <span style={{ fontSize: "12px", color: "var(--k-orange-ink)" }}>
                  Match Score: {Math.round(worker.match_score || 0)}%
                </span>
              </div>
            ))
          )}
