...
def upsert_chat_data(db: Session, booking_chat_id: int, user_id: int, fields: dict):
    ...
    db_profile = (
        db.query(model.CustomerChatData)
        .filter(model.CustomerChatData.booking_chat_id == booking_chat_id)
        .first()
    )
    ...
    if db_profile:
        ...
        db_profile = model.CustomerChatData(
            user_id=user_id, booking_chat_id=booking_chat_id, **fields
        )
    db.add(db_profile)
    return db_profile
...
def upsert_job(db: Session, booking_chat_id: int, user_id: int, fields: dict):
    ...
    db_job = (
        db.query(model.Job).filter(model.Job.booking_chat_id == booking_chat_id).first()
    )
    ...
    if db_job:
        ...
        db_job = model.Job(
            booking_chat_id=booking_chat_id, customer_id=user_id, **fields
        )
        db.add(db_job)
...
def get_job_by_id(db: Session, job_id: int, customer_id: int):
    ...
    return (
        db.query(model.Job)
        .filter(model.Job.id == job_id, model.Job.customer_id == customer_id)
        .first()
    )


def get_all_jobs_for_customer(
    db: Session, customer_id: int, skip: int = 0, limit: int = 50
):
    """Fetch all jobs for a specific customer with pagination."""
    return (
        db.query(model.Job)
        .filter(model.Job.customer_id == customer_id)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def get_workers_by_category(category: str, db: Session):
    ...
    try:
        ...
        workers = (
            ...
            .filter(model.WorkerProfile.job_category.ilike(category))
            .all()
        )
        ...
        raise HTTPException(
            status_code=500, detail="Database query failed while fetching workers."
        )
