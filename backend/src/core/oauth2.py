...
from fastapi.security import (
    OAuth2PasswordBearer,  # this grabs token from the request header and verifies it if that is token or not, if not returns 401 error
)
...
oauth2_schema = OAuth2PasswordBearer(
    tokenUrl="login"
)  # this creates an instance which will be used to handle the token, and tokenUrl is the endpoint where the user will send their username and password to get a token
...
def create_access_token(data: dict):
    to_encode = (
        data.copy()
    )  # creatind copy of data to avoid modifying the original data
    expire = (
        datetime.now(timezone.utc) + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    )  # for expiry it will be current time + the number of minutes specified in the settings
    to_encode.update({"exp": expire})  # this add the expiry time to the token data
    encoded_jwt = jwt.encode(
        to_encode, SECRET_KEY, algorithm=ALGORITHM
    )  # this will encode the token using the secret key and the algorithm specified in the settings
...
def verify_access_token(token: str, credentials_exception: HTTPException):
    try:
        ...
        )  # this will decode the token using the secret key and the algorithm specified in the settings, if the token is invalid, it will raise an error
        id = str(
            payload.get("user_id")
        )  # this will get the user_id from the token payload, which we will use to identify the user in future requests
        if id is None:
            raise credentials_exception
        token_data = TokenData(
            user_id=id
        )  # this is about validating token data, double checking if the token contains the user_id, if it does not contain the user_id, it will raise an error

    except (
        jwt.PyJWTError
    ):  # this wioll catch error occured during decoding the toke and raise error
        raise credentials_exception
...
def get_current_user(
    ...
    )  # this will verify the token and return the token data, which contains the user_id, if the token is valid, otherwise it will raise an error
    user = (
        db.query(model.User).filter(model.User.id == token_data.user_id).first()
    )  # this will query the database to get the user with the user_id contained in the token, if the user does not exist, it will return None
