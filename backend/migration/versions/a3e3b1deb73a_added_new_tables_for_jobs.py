...
def upgrade() -> None:
    ...
    op.drop_index(op.f('ix_ _id'), table_name=' ')
    op.drop_table(' ')
...
def downgrade() -> None:
    ...
    op.drop_geospatial_index('idx_customer_chat_data_location', table_name='customer_chat_data', postgresql_using='gist', column_name='location')
    ...
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f(' _user_id_fkey'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f(' _pkey'))
    )
    op.create_index(op.f('ix_ _id'), ' ', ['id'], unique=False)
    # ### end Alembic commands ###
